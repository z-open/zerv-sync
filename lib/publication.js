...
function Publication(name, fetchFn, dataNotifications, options= {}) {
    ...
    this.incrementalEnabled = true; //options.incremental == true;
}
...
Publication.prototype.notifySubscriptions = function(tenantId, dataNotification, notificationType, objects, options) {
    ...
    objects = _.map(objects, zjsonBin.jsonify);
                ...
                .forEach(function(subscription) {
                    ...
                    .then(function(records) {
                        ...
                        _.forEach(records, function(record) {
                            try {
                                // make sure that the subscription is matching the notification params otherwise it might a record that might need to be removed
                                subRecords.push({
                                    record,
                                    type: !options.forceNotify === true && !subscription.checkIfMatch(record, dataNotification) ? 'REMOVAL' : notificationType
                                });
                            } catch (err) {
...
Publication.prototype.getRecordBase = function(tenantId, id, revision) {
    return this.recordBaseCache[getRecordBaseHashkey(tenantId, id, revision)];
};

Publication.prototype.cacheRecordBase = function(tenantId, record) {
    this.recordBaseCache[getRecordBaseHashkey(tenantId, record.id, record.revision)] = zjsonBin.jsonify(record);
};

Publication.prototype.cacheIncrementalChanges = function(tenantId, id, increment, toRevision, fromRevision) {
   
};

Publication.prototype.getCachedIncrementalChanges = function(tenantId, id, revision, toRevision, fromRevision) {
    return null;
};

function getRecordBaseHashkey(tenantId, id, revision) {
    return tenantId + '[' + id + ']' + revision;
}
