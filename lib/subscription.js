...
function Subscription(user, subscriptionId, publication, params) {
    ...
    function flush(isAllRecords) {
        let recordsToProcess = readQueue();
        ...
        thisSub.socket.emit('SYNC_NOW', thisSub.transport.disabled ? notificationObj : thisSub.transport.serialize(notificationObj, true), function(response) {
    ...
    function emitChanges(records, notificationType) {
        ...
        _.forEach(records, function(record) {
            if (notificationType === 'REMOVAL') {
                ...
                // }
                // record.timestamp.remove = new Date();
                record = {
    ...
    function addToQueue(record) {
        let rev = getRecordRevision(record);
...
//     let r = [];
//     for (let publicationName in publications) {
...
//         let socketSubscriptions = socket.subscriptions;
//         socket.subscriptions = [];
