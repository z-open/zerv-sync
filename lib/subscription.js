...
function Subscription(user, subscriptionId, publication, params) {
  ...
  function buildLightRecordForRemoval(record) {
    return {
      ...
      revision: recalculaterRevision(record),
   ...
   * Side effect: if a user is currently modifying this object while the record is notified,
   * there could be a revision conflict. Probability is extremly low.
   * @param {Object} record
   * @return {Number} revision with decimal part based on timestamp
   */
  function recalculaterRevision(record) {
