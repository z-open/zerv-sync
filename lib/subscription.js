...
function Subscription(user, subscriptionId, publication, params) {
    ...
    function flush(isAllRecords) {
        ...
        if (publication.incrementalEnabled) {
            notificationObj.incremental = true;
            notificationObj.records = getRecordIncrementalChanges(recordsToProcess);
        } else {
    ...
    // if sub option has incremental then send incrementals otherwise
    // just the data as usual
    // prevent recalculating difference to often and jsonify
    // one pass jsonify to remove stringfy thing
    /**
     * get the incremental changes since the last sync with the subscription
     * 
     * @param {Array} recordsToProcess 
     */
    function getRecordIncrementalChanges(recordsToProcess) {
        return _.map(recordsToProcess, record => {
 ...
 */
            ...
            let incr = publication.getCachedIncrementalChanges(thisSub.tenantId, record.id, record.revision, clientStateVersion);

            if (!incr) {
                const base = publication.getRecordBase(thisSub.tenantId, record.id, clientStateVersion);
                if (!base) {
                    // there is no reference to calculate incremental change
                ...
                publication.cacheIncrementalChanges(thisSub.tenantId, record.id, incr, record.revision, clientStateVersion);
            }
    ...
    function addToCache(records) {
        records.forEach(function (record) {
            ...
            if ((!clientStateRevision || clientStateRevision < recordRev) && !record.remove) {
                ...
                if (publication.incrementalEnabled) {
                    publication.cacheRecordBase(thisSub.tenantId, record);
                }
            }
        });
