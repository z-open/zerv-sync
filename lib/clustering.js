...
function syncWithRedis(processRecordNotifications) {
    ...
    function onMessage(channel, message) {
        ...
        if (json.zervId !== zervId) {
            logger.debug('Received Cluster Sync notification %b', ChannelSplitterNode);
    ...
    function listenToTenantNotification(tenantId) {
        if (!notificationListenerCounts[tenantId]) {
            notificationListenerCounts[tenantId] = true;
            logger.debug("listen to cluster notifications for tenant %b", tenantId)
    ...
    function releaseTenantNotificationListener(tenantId) {
        delete notificationListenerCounts[tenantId];
        logger.debug("Stop listening to cluster notifications for tenant %b", tenantId)
