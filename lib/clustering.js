...
function syncWithRedis(processRecordNotifications, transport) {
    ...
    redis.on('message', (channel, message) => _processMessage(transport, channel, message, processRecordNotifications));
...
function _processMessage(transport, channel, message, processRecordNotifications) {
    let json = null;
    try {
    ...
    // redis messages originating from the same server are not processed again since they are already processed.
        ...
        logger.error('Error while processing received notification on Redis channel %b: %s', channel, err.message, err);
    }
    return json;
}
