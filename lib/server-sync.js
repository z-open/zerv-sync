 ...
 */
function publish(name, fetchDataFn, dataNotification, options) {
    ...
    var enhancedOptions = _.assign({ cache }, options); // later on, add options to configure or deactivate cache
    publications[name] = new Publication(name, fetchDataFn, dataNotifications, enhancedOptions);
    return sync;
}
                        ...
                        try {
                            ...
                            if (options.forceNotify === true || subscription.checkIfMatch(object)) {
                                ...
                                if (publication.always) {
                                    ...
                                    cache.update(publication, tenantId, subscription.params, object, notificationType);
...
function Publication(name, fetchFn, dataNotifications, options) {
    ...
    this.fn = options.cache ? fetchAllData : fetchFn;

    function fetchAllData(tenantId, user, params) {
        var sub = this;
        var data = options.cache.getData(this, tenantId, params);
        if (data) {
            return Promise.resolve(data);
        }
        return fetchFn(tenantId, user, params)
            .then(function (data) {
                return options.cache.reset(sub, tenantId, params, data);
            });
    };
}
