...
const cluster = require('./clustering')(processRecordNotifications);
...
function subscribe(user, socket, subscriptionId, publicationName, params) {
    ...
    if (subscriptionId) {
        subscription = publication.addSubscription(user, subscriptionId, publication, params);
    ...
    if (cluster) {
        cluster.listenToTenantNotification(user.tenantId);
        subscription.onRelease(function (tenantId) {
            if (getActiveSubscriptions(tenantId) === 0) {
...
function getActiveSubscriptions(tenantId) {
    ...
    return !tenantId ? subs : _.filter(subs, { tenantId });
}
