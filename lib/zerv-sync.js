...
if (!sync) {
    ...
    const cluster = require('./clustering')(processRecordNotifications, zervCore.transport);
    ...
    function subscribe(user, socket, subscriptionId, publicationName, params) {
        ...
        let publication = findPublicationByName(publicationName);
        if (subscriptionId) {
        ...
        subscription.onRelease(function(tenantId) {
        ...
        if (listeners) {
            listeners.forEach(function(callback) {
                _.forEach(records, function(record) {
                    ...
                    try {
                        let r = callback(tenantId, record, notificationType, options);
                        if (r && r.then) {
