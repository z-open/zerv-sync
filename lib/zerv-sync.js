...
if (!sync) {
    ...
    sync._processExternalRecordNotifications = _processExternalRecordNotifications;

    const cluster = clusterService.syncWithRedis(sync._processExternalRecordNotifications, zervCore.transport);
     ...
     *  @property {boolean} options.allServers set to true to notify all servers via the socket 
     *                                         By default false, only servers handling the notified tenants receive the data.
     *  @property {String} options.onlyUserId, if provided only the subscription matching the user id will be notified via the socket
     * 
     *  @returns {Promise<number>} number of notifications which are scheduled to be emitted to the interested subscriptions
     * 
     */
    function notifyRecordActivity(tenantId, dataNotification, notificationType, objects, options) {
        ...
        // the notification was originated locally
        options = _.assign({clusterSrc: false}, options);
    ...
    function _processExternalRecordNotifications(tenantId, dataNotification, notificationType, objects, options) {
     ...
     *  @param {String} notificationType
     *  @param {array} records
     *  @param {Object} options should not be nil
     *  @property {String} options.clusterSrc, if true, the notification was received via the socket otherwise it was originated locally
     * 
     */
    function notifyChangeListeners(tenantId, dataNotification, notificationType, records, options) {
        ...
        if (listeners) {
            _.forEach(listeners, (listener) => {  
                // if the notif comes from the cluster,
                //      execute callback except if listener is set to listen to notifs that originated locally only
                // if the notif comes from this server, 
                //      execute callback
                if (!options.clusterSrc || !listener.localOnly) {
