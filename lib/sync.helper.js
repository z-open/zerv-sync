...
async function processUpdate(data, fetchObjectRevision, updateObjectData, saveUpdatedObject, handleConflict) {
    const currentObj = await fetchObjectRevision();
    ...
    // the revision received is based on old data, the currentObj is ahead
    if (data.revision < currentObj.revision) {
        ...
        const originObj = await fetchObjectRevision(data.revision);
